...
\begin{abstract}
This paper explores the difference in result when generating twitter messages (tweets) that seem to come from a former Swedish foreign minister, Carl Bildt, using two natural language generation based techniques that build on each other.
The outcome of the methods were tweets that seemed to origin from Carl Bildt with a subject that circulated around foreign politics. 
The most reliable ones however were mainly copied directly from his feed, a problem we headed out to solve with this method.
We used Markov chains with and without constraints to generate tweets and we give suggestions on improvement of our technique.
\end{abstract}
...
We contrast this method using constrained Markov chains. 
...
We have implemented a unigram of POS on to a bigram in order to observe the difference in the result. 
...
Our experiments however will only use a unigram for the transition matrix, even if the Markov chain of words from the corpus is longer, 
...
Because we use a bigram, three successive words will always be chosen from the corpus but anything more than that makes it less unique.
...
Thus this sentences uniqueness is $\frac{9 - 2}{9} \approx$ 78 percent.
...
When generating tweets two methods are used, first the bigram gets to select which words to use entirely on its own (Markov chain). Then the process is repeated but this time the bigram is constrained by which lexical class of words it is allowed to choose from, that class being whichever one the transition matrix has decided should follow (constrained Markov chain).
